...
export default function QuickEstimatorResults({
  ...
  // Ограничения для балки
  if (frameType === "beam") {
    if (W > 7 || H > 7.5) blockType1 = "Балка: пролет до 7м, высота до 7.5м";
    if (W > 7) blockType2 = "Балка: пролет до 7м"; // Для Типа 2 высота не ограничена
  }
  
  // Ограничения для фермы
  if (frameType === "truss" && W > 30) {
    blockType1 = "Ферма: пролет до 30м";
    blockType2 = "Ферма: пролет до 30м";
  }
  
  // Жесткое ограничение по высоте для ЛСТК-колонн (Только Тип 1)
  if (H > 7.5) {
    blockType1 = "Тип 1: высота колонн не более 7.5м";
  }

  // Ограничения по кранам
  if (hasAnyCrane) {
